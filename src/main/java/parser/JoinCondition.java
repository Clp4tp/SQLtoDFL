...
public class JoinCondition {
    ...
    private List<JoinCondition> neighbors = new ArrayList<JoinCondition>();
	...
	if (this.left[0].equals(neighbor.left[0]) &&  this.left[1].equals(neighbor.left[1])  || this.left[0].equals(neighbor.right[0]) && this.left[1].equals(neighbor.right[1])) {
	    System.out.println("leftConn");
	    return true;
	}
	if (this.right[0].equals(neighbor.left[0]) &&  this.right[1].equals(neighbor.left[1])  || this.right[0].equals(neighbor.right[0]) && this.right[1].equals(neighbor.right[1])  ) {
	    System.out.println("rightConn");
	    return true;
	}
	...
	if(query.getFromTables().size()==2){
	 if (graph.size()>=1)  {
	     //so if we have 2 tables and actually the graph is bigger than one we will definitely have a joining attribute.
	     return graph.get(0).left[1]; //pick any since left[1] == right[1]
	 }  
	}
	else{
