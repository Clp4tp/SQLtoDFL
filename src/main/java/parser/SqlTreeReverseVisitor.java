...
public class SqlTreeReverseVisitor<R> implements SqlVisitor<R> {
	...
	List<List<String>> functionsToTables = new ArrayList<>(); // new
																// ArrayList<>();//HashMultimap.create();
	...
	public R visit(SqlCall call) {
		...
		if (call instanceof SqlBasicCall) {
			...
			if (InterestingOperators.contains(opString.toLowerCase())) {
				...
				if (table.equals("*")) {
					list.add(opString);
					list.add(call.getOperandList().get(0).toString().replace("`", ""));
					functionsToTables.add(list);
					// map.put(opString,
					// call.getOperandList().get(0).toString().replace("`",
					// ""));
